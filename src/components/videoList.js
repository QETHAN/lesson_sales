export default function VideoList() {
  return <div className="container lg:max-w-screen-lg mx-auto px-5 sm:px-0 pb-10">
  <h1 className="py-10 text-2xl font-bold text-gray-800">YouTube频道</h1>
  <ul className="flex flex-col sm:flex-row flex-wrap justify-between">
    <li className="basis-[calc(50%-10px)]">
      <a
        className="flex flex-col sm:flex-row"
        href=""
        aria-label="用 iPhone 14 拍摄：摄影佳作激发创作灵感，定格精彩人像、自然与城市风光 - 新闻稿 - 发表时间 2035 年 9 月 1 日"
      >
        <div className="w-[132px] h-[132px] rounded-lg overflow-hidden" aria-hidden={true}>
          <img
            class="image object-cover h-full hover:scale-[1.03]"
            data-src="https://www.apple.com.cn/newsroom/cn/images/product/iphone/lifestyle/Apple_Shot-on-iPhone-14-models_iPhone-14-Pro-Max-with-the-Main-Camera-by-Xiaobei-Fuzhou_12192022_Full-Bleed-Image.jpg.xlarge_2x.jpg"
            alt="lt"
            src="https://www.apple.com.cn/newsroom/cn/images/product/iphone/lifestyle/Apple_Shot-on-iPhone-14-models_iPhone-14-Pro-Max-with-the-Main-Camera-by-Xiaobei-Fuzhou_12192022_Full-Bleed-Image.jpg.xlarge_2x.jpg"
          />
        </div>
        <div className="flex-1 p-6">
          <h2 className="text-xl font-bold line-clamp-6">
            用 iPhone 14
            拍摄：摄影佳作激发创作灵感，定格精彩人像、自然与城市风光
          </h2>
        </div>
      </a>
    </li>

    <li className="basis-[calc(50%-10px)]">
      <a
        className="flex flex-col sm:flex-row"
        href=""
        aria-label="用 iPhone 14 拍摄：摄影佳作激发创作灵感，定格精彩人像、自然与城市风光 - 新闻稿 - 发表时间 2035 年 9 月 1 日"
      >
       <div className="w-[132px] h-[132px] rounded-lg overflow-hidden" aria-hidden={true}>
          <img
            class="image object-cover h-full rounded-lg hover:scale-[1.03]"
            alt="lt"
            src="https://cdn2.unrealengine.com/student-to-3d-artist-header-1920x1080-73d477520f8e.jpg?resize=1&w=1920"
          />
        </div>
        <div className="flex-1 p-6">
          <h2 className="text-xl font-bold line-clamp-6">
            用 iPhone 14
            拍摄：摄影佳作激发创作灵感，定格精彩人像、自然与城市风光
          </h2>
        </div>
      </a>
    </li>

    <li className="basis-[100%] my-8 h-px bg-gray-200"></li>

    <li className="basis-[calc(50%-10px)]">
      <a
        className="flex flex-col sm:flex-row"
        href=""
        aria-label="用 iPhone 14 拍摄：摄影佳作激发创作灵感，定格精彩人像、自然与城市风光 - 新闻稿 - 发表时间 2035 年 9 月 1 日"
      >
        <div className="w-[132px] h-[132px] rounded-lg overflow-hidden" aria-hidden={true}>
          <img
            class="image object-cover h-full rounded-lg hover:scale-[1.03]"
            alt="lt"
            src="https://www.apple.com.cn/newsroom/images/values/environment/Apple-Earth-Day-India-mangrove-Alibaug-canoe_Full-Bleed-Image.jpg.large_2x.jpg"
          />
        </div>
        <div className="flex-1 p-6">
          <h2 className="text-xl font-bold line-clamp-6">
            用 iPhone 14
            拍摄：摄影佳作激发创作灵感，定格精彩人像、自然与城市风光
          </h2>
        </div>
      </a>
    </li>

    <li className="basis-[calc(50%-10px)]">
      <a
        className="flex flex-col sm:flex-row"
        href=""
        aria-label="用 iPhone 14 拍摄：摄影佳作激发创作灵感，定格精彩人像、自然与城市风光 - 新闻稿 - 发表时间 2035 年 9 月 1 日"
      >
       <div className="w-[132px] h-[132px] rounded-lg overflow-hidden" aria-hidden={true}>
          <img
            class="image object-cover h-full rounded-lg hover:scale-[1.03]"
            alt="lt"
            src="https://cdn2.unrealengine.com/student-to-3d-artist-header-1920x1080-73d477520f8e.jpg?resize=1&w=1920"
          />
        </div>
        <div className="flex-1 p-6">
          <h2 className="text-xl font-bold line-clamp-6">
            用 iPhone 14
            拍摄：摄影佳作激发创作灵感，定格精彩人像、自然与城市风光
          </h2>
        </div>
      </a>
    </li>

    <li className="basis-[100%] my-8 h-px bg-gray-200"></li>

    <li className="basis-[calc(50%-10px)]">
      <a
        className="flex flex-col sm:flex-row"
        href=""
        aria-label="用 iPhone 14 拍摄：摄影佳作激发创作灵感，定格精彩人像、自然与城市风光 - 新闻稿 - 发表时间 2035 年 9 月 1 日"
      >
        <div className="w-[132px] h-[132px] rounded-lg overflow-hidden" aria-hidden={true}>
          <img
            class="image object-cover h-full rounded-lg hover:scale-[1.03]"
            alt="lt"
            src="https://www.apple.com.cn/newsroom/images/values/environment/Apple-Earth-Day-India-mangrove-Alibaug-canoe_Full-Bleed-Image.jpg.large_2x.jpg"
          />
        </div>
        <div className="flex-1 p-6">
          <h2 className="text-xl font-bold line-clamp-6">
            用 iPhone 14
            拍摄：摄影佳作激发创作灵感，定格精彩人像、自然与城市风光
          </h2>
        </div>
      </a>
    </li>
  </ul>
</div>
}